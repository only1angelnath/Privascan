...
import { getScore, getScoreHistory, RateLimitError, type ScoreResult, type HistoryPoint } from '@/lib/api'
...
const TVL_TIER_DESC: Record<string,string> = {
  Whale:'>$100M — institutional-grade liquidity',Large:'>$10M — well-established protocol',
  Medium:'>$1M — growing adoption',Small:'>$100K — early stage',
  Micro:'<$100K — minimal liquidity, high risk',Unknown:'TVL data unavailable',
}

function RateLimitBanner({ retryAfter }: { retryAfter: number }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 pt-20">
      <div className="glass rounded-2xl p-12 text-center max-w-lg w-full" style={{borderColor:'rgba(245,158,11,0.3)'}}>
        <div className="font-orbitron text-3xl font-black mb-2" style={{color:'#f59e0b'}}>429</div>
        <div className="font-orbitron text-lg font-bold text-white mb-4">RATE LIMIT REACHED</div>
        <p className="font-mono text-sm text-slate-400 mb-2 leading-relaxed">
          Anonymous users are limited to <span className="text-white">2 req/min</span> and <span className="text-white">10 req/hr</span>.
        </p>
        <p className="font-mono text-sm text-slate-400 mb-8 leading-relaxed">
          Get a free API key for <span className="text-white">15 req/min + 500 req/hr</span> — takes 30 seconds via Telegram.
        </p>
        <a href="https://t.me/PrivaScanBot" target="_blank" rel="noreferrer"
          className="inline-block font-orbitron text-sm font-bold px-8 py-3.5 rounded-lg tracking-wider hover:opacity-90 transition-all mb-4"
          style={{background:'#00d4ff',color:'#0a0f1e'}}>
          GET FREE KEY → @PrivaScanBot
        </a>
        {secs > 0 && <p className="font-mono text-xs text-slate-600 mt-4">or retry in {secs}s</p>}
        <div className="mt-6 pt-6" style={{borderTop:'1px solid rgba(255,255,255,0.06)'}}>
          <Link href="/keys" className="font-mono text-xs text-slate-500 hover:text-slate-300 transition-colors">
            Already have a key? Enter it on the keys page →
          </Link>
        </div>
      </div>
    </div>
...
export default function ScorePage() {
  ...
  const [rateLimitSecs, setRLSecs]    = useState<number|null>(null)
  ...
  useEffect(() => {
    ...
    Promise.all([getScore(p.chain,p.address), getScoreHistory(p.chain,p.address)])
      ...
      .catch(e => {
        if (e instanceof RateLimitError) setRLSecs(e.retryAfter||60)
        else setErr(e.message||'Failed to score contract')
      })
      .finally(() => { clearInterval(sid); setLoading(false) })
    return () => clearInterval(sid)
  }, [p.chain, p.address])

  if (!loading && rateLimitSecs !== null) return <RateLimitBanner retryAfter={rateLimitSecs} />
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 pt-20">
      <div className="glass rounded-2xl p-12 text-center max-w-lg w-full" style={{borderColor:'rgba(0,212,255,0.2)'}}>
        ...
        <div className="font-mono text-xs tracking-widest" style={{color:'#00d4ff'}}>{STAGES[stage]}</div>
    ...
    <div className="max-w-7xl mx-auto px-6 pt-28 pb-16">
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-6">
        ...
        <div className="glass rounded-xl p-6">
          ...
          {BARS.map(b => <SubScoreBar key={b.key} label={b.label} score={(ss as Record<string,number>)[b.key]||0} weight={b.w}/>)}
        </div>
        <div className="glass rounded-xl p-6">
          ...
          <div className="mt-4 space-y-1">
            <div className="flex justify-between font-mono text-xs text-slate-500"><span>Chain</span><span className="text-slate-300">{data.chain} #{data.chain_id}</span></div>
            <div className="flex justify-between font-mono text-xs text-slate-500"><span>Verified</span><span style={{color:data.details.code.is_verified?'#22c55e':'#ef4444'}}>{data.details.code.is_verified?'YES':'NO'}</span></div>
            <div className="flex justify-between font-mono text-xs text-slate-500"><span>TVL</span><span className="text-slate-300">{data.details.liquidity.tvl_usd!=null?`$${(data.details.liquidity.tvl_usd/1e6).toFixed(1)}M`:'—'}</span></div>
            <div className="flex justify-between font-mono text-xs text-slate-500"><span>High findings</span><span style={{color:data.details.code.high_count>0?'#ef4444':'#22c55e'}}>{data.details.code.high_count}</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="glass rounded-xl p-6">
          <div className="font-mono text-xs text-slate-500 tracking-widest mb-4">OWNERSHIP FLAGS</div>
          {(data.details.ownership.flags||[]).length===0
            ? <p className="font-mono text-xs text-slate-600">No flags raised.</p>
            : <div className="space-y-2">{(data.details.ownership.flags as string[]).map(f=>{
                const meta=FLAG_LABELS[f]||{label:f,impact:'',color:'#94a3b8'}
                return(<div key={f} className="flex items-center justify-between glass rounded-lg px-3 py-2" style={{borderColor:`${meta.color}33`}}>
                  <span className="font-mono text-xs" style={{color:meta.color}}>{meta.label}</span>
                  <span className="font-mono text-xs text-slate-500">{meta.impact}</span>
                </div>)
              })}</div>
          }
        </div>
        <div className="glass rounded-xl p-6">
          <div className="font-mono text-xs text-slate-500 tracking-widest mb-4">LIQUIDITY DETAIL</div>
          <div className="space-y-2">
            <div className="flex justify-between font-mono text-xs text-slate-500"><span>TVL Tier</span><span className="text-white">{data.details.liquidity.tvl_tier||'—'}</span></div>
            <div className="font-mono text-xs text-slate-600 mt-1">{TVL_TIER_DESC[data.details.liquidity.tvl_tier]||''}</div>
            <div className="flex justify-between font-mono text-xs text-slate-500 mt-3"><span>Source</span><span className="text-slate-300">{data.details.liquidity.tvl_source||'—'}</span></div>
            <div className="flex justify-between font-mono text-xs text-slate-500"><span>Confidence</span><span className="text-slate-300">{data.details.liquidity.tvl_confidence||'—'}</span></div>
          </div>
        </div>
      </div>
    </div>
